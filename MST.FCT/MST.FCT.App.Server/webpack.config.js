...
const collectEntries = () => {
    const fileList = glob.sync(`wwwroot/!(${conf.exclude.join('|')})/**/!(_)*.@(js|es6)`) || []
    return fileList.filter(p => !/\.dist\.(?:js|css)$/.test(p)).reduce((entries, file) => {
        const filePath = file.replace(/\\/g, '/')
        return { ...entries, [filePath.replace(/^wwwroot\/|\.(?:js|es6)$/g, '')]: `./${filePath}` }
    }, {})
}
